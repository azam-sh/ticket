...
const MoviesPage = ({ events }: { events: Event[] }) => {
  ...
  const movies = events.filter((event) => event.category === "movie");
  const foundEvents = movies.filter((event) =>
  ...
  return (
    <>
      <div className="bg-gradient">
        <div className="flex flex-col container mx-auto px-[30px] sm:px-[80px] pb-[10px] md:px-[50px] lg:px-[80px] xl:px-[100px] 2xl:px-[120px] pt-[10px] md:pb-[80px] lg:pb-[100px]">
          <Navbar />
      ...
      <div className="container mx-auto sm:px-[30px] md:px-[50px] lg:px-[80px] xl:px-[100px] 2xl:px-[120px] pt-[40px] pb-[60px]">
        <EventsSection events={movies} title={"Кино"} />
      </div>
      <Footer />
    </>
  );
